                                ...
                                <tr data-id="{{ $product->id }}">
                                    ...
                                    {{-- <td class="text-center">
                                        <input type="checkbox" class="form-check-input toggle-menu"
                                            data-id="{{ $product->id }}" {{ $product->is_menu ? 'checked' : '' }}>
                                    </td> --}}
                                    <td class="text-center">
                                        <input type="checkbox" class="form-check-input toggle-home"
                                            data-id="{{ $product->id }}" {{ $product->is_home ? 'checked' : '' }}>
                                    </td>
                                    <td>
                                        <a
                                            class="d-flex align-items-center">
                                            @if (!empty($product->category) && !empty($product->category->category_image))
                                                <img src="{{ asset('storage/' . $product->category->category_image) }}"
                                                    class="user-img me-2" alt="Category Image">
                                            @else
                                                <img src="{{ asset('admin/images/logo-icon.png') }}" class="user-img me-2"
                                                    alt="Default Image">
                                            @endif
                                            <div class="user-info">
                                    ...
                                    <td>
                                        <a
                                            ...
                                            @endif
                                            <div class="user-info">
                                                <p class="user-name mb-0">{{ $product->product_name }}</p>
                                            </div>
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <button type="button"
                                            class="btn btn-{{ $product->status == 'Active' ? 'success' : 'danger' }} btn-sm radius-30 px-4">
                                            {{ $product->status }}
                                        </button>
                                    </td>
                                    <td class="text-center"> {{ $product->updated_at->format('d-m-Y h:i A') }}</td>
        ...
        function confirmAndToggle($checkbox, dataField) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.post("{{ route('admin.product.toggleCheckbox') }}", postData, function(response) {
